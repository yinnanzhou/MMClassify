...
def plotMain(
    data_1dfft, data_1dfft_dyn, range_angle, angle_diff, angle_diff_filtered, t_f_Zxx
):
    numFrames, numSamples = data_1dfft.shape
    t, f, Zxx = t_f_Zxx
    X, Y = np.meshgrid(
        np.linspace(1, numSamples, numSamples), np.linspace(1, numFrames, numFrames)
    )
...
def plotSTFT_new(t, f, Zxx, vmin=-60, vmax=-15):
    plt.figure(figsize=(8, 6))
    plt.imshow(
        10 * np.log10(np.abs(Zxx)),
        aspect="auto",
...
def saveSTFT(t, f, Zxx, save_path, wordIndex, fileIndex, personIndex, txIndex):
    ...
    for i in range(num_subplots):
        ...
        plt.imshow(
            ...
            aspect="auto",
            extent=[start_time, end_time, f.min(), f.max()],
            origin="lower",
            cmap="jet",
            interpolation="bilinear",
            vmin=-60,
...
def saveSTFT_hdf5_mmwave(
    Zxx, first_indices, save_path, wordIndex, personIndex, repeatIndex
):
    for fileIndex in range(len(first_indices) - 1):
        plt.figure()
        Zxx_split = Zxx[:, first_indices[fileIndex] : first_indices[fileIndex + 1]]
        t_split = np.arange(0, (Zxx.shape[1]) * 0.018, 0.018)
        f_split = np.linspace(0, 16000 * Zxx.shape[0] / 481, num=Zxx.shape[0])

        plt.imshow(
            10 * np.log10(np.abs(Zxx_split)),
            aspect="auto",
            extent=[t_split[0], t_split[-1], f_split[0], f_split[-1]],
            origin="lower",
            cmap="jet",
            interpolation="bilinear",
            vmin=10,
        ...
        plt.axis("off")  # Remove axes
        plt.savefig(
            f"{save_path}/{str(wordIndex)}_{str(fileIndex)}_{str(personIndex)}_{str(repeatIndex)}.png",
            bbox_inches="tight",
            pad_inches=0,
            dpi=300,
        )
...
def saveSTFT_hdf5_audio(
    Zxx, first_indices, save_path, wordIndex, personIndex, repeatIndex, vmin=10, vmax=60
):
    for fileIndex in range(len(first_indices) - 1):
        plt.figure()
        Zxx_split = Zxx[:, first_indices[fileIndex] : first_indices[fileIndex + 1]]
