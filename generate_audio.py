...
with h5py.File(file_path, "r") as hdf5_file:
    ...
    noise_list = [
        i[11:] for i in hdf5_file["various_noise"].keys() if "spec_NOISE_" in i
    ]
    for group_name in group_names:
        try:
            preloaded_data[group_name] = {
                "spec_mmwave": torch.from_numpy(
                    hdf5_file[group_name]["spec_mmwave"][:]
                ).to(torch.complex64),
                "spec_audio": torch.from_numpy(
                    hdf5_file[group_name]["spec_audio"][:]
                ).to(torch.complex64),
                "columns": hdf5_file[group_name]["columns"][:],
            }
        except KeyError as e:
            print(f"KeyError for group {group_name}: {e}")
    preloaded_noise = {
        noise: {
            "spec_noise": torch.from_numpy(
...
save_path = r"/home/mambauser/MMClassify/data/dataPng/continuous_audio"
...
for group_name in tqdm(group_names, desc="Processing Groups"):
    Zxx = preloaded_data[group_name]["spec_audio"][:, :]
